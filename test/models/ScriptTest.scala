...
class ScriptTest extends FlatSpec {
  ...
  it should "p2pkh" in {
    ...
    script_sig.serialize(out2)
    val result2 = ByteVector(out2.toByteArray)
    assert(script_sig.`type` == "p2pkh sig")
    assert(result2.toHex == script_sig_raw)
  ...
  it should "p2sh" in {
    ...
    val out2 = new ByteArrayOutputStream()
    script2.serialize(out2)
    val result2 = ByteVector(out2.toByteArray)
    assert(result2.toHex == h2)
